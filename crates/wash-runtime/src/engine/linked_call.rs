...
async fn build_ctx_from_template(
    ...
    // `wasi:sockets` is linked unconditionally, while the HTTP egress allowlist
    // governs only `wasi:http`. A literal post-DNS address can therefore bypass
    // that allowlist unless raw TCP is denied independently here.
